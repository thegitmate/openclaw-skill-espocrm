#!/usr/bin/env python3
"""
EspoCRM Task operations.

Usage:
  python -m scripts.task list [--limit 20] [--offset 0] [--where FIELD VALUE]...
  python -m scripts.task get <id>
  python -m scripts.task search --FIELD VALUE
  python -m scripts.task create --name "Follow up" [--dateEnd 2025-12-31] [--FIELD VALUE]...
"""
...
ENTITY = "Task"
SUMMARY_FIELDS = ["id", "name", "status", "priority", "dateEnd", "assignedUserName", "parentType", "parentName"]
...
def create_record(args):
    ...
    while i < len(args):
        if args[i].startswith("--") and i + 1 < len(args):
            ...
            i += 1
    if not payload:
        error("No fields provided. Use --name 'Task name'")
    api_url, api_key, cf_id, cf_secret = get_config()
...
def update_record(args):
    if not args:
        error("Missing task ID. Usage: update <id> --field value")
    record_id = args[0]
    payload = {}
    i = 1
    while i < len(args):
...
def delete_record(args):
    if not args:
        error("Missing task ID. Usage: delete <id>")
    record_id = args[0]
    api_url, api_key, cf_id, cf_secret = get_config()
