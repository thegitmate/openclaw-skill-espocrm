#!/usr/bin/env python3
"""
EspoCRM Contact operations.

Usage:
  python -m scripts.contact list [--limit 20] [--offset 0] [--where FIELD VALUE]...
  python -m scripts.contact get <id>
  python -m scripts.contact search --FIELD VALUE
  python -m scripts.contact create --firstName John --lastName Doe --emailAddress john@example.com [--FIELD VALUE]...
"""
...
ENTITY = "Contact"
SUMMARY_FIELDS = ["id", "firstName", "lastName", "emailAddress", "phoneNumber", "accountName", "city", "country"]
...
def create_record(args):
    ...
    while i < len(args):
        if args[i].startswith("--") and i + 1 < len(args):
            ...
            i += 1
    if not payload:
        error("No fields provided. Use --firstName John --lastName Doe --emailAddress john@example.com")
    api_url, api_key, cf_id, cf_secret = get_config()
...
def update_record(args):
    if not args:
        error("Missing contact ID. Usage: update <id> --field value")
    record_id = args[0]
    payload = {}
    i = 1
    while i < len(args):
...
def delete_record(args):
    if not args:
        error("Missing contact ID. Usage: delete <id>")
    record_id = args[0]
    api_url, api_key, cf_id, cf_secret = get_config()
