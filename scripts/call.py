#!/usr/bin/env python3
"""
EspoCRM Call operations.

Usage:
  python3 -m scripts.call list [--limit 20] [--offset 0] [--where FIELD VALUE]...
  python3 -m scripts.call get <id>
  python3 -m scripts.call search --FIELD VALUE
  python3 -m scripts.call create --name "Call with John" --dateStart "2025-12-01 10:00:00" [--FIELD VALUE]...
  python3 -m scripts.call update <id> --FIELD VALUE [--FIELD VALUE]...
  python3 -m scripts.call delete <id>
"""
...
def update_record(args):
    if not args:
        error("Missing call ID. Usage: update <id> --field value")
    record_id = args[0]
    payload = {}
    i = 1
    while i < len(args):
...
def delete_record(args):
    if not args:
        error("Missing call ID. Usage: delete <id>")
    record_id = args[0]
    api_url, api_key, cf_id, cf_secret = get_config()
