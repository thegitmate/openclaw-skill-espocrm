#!/usr/bin/env python3
"""
EspoCRM Meeting operations.

Usage:
  python -m scripts.meeting list [--limit 20] [--offset 0] [--where FIELD VALUE]...
  python -m scripts.meeting get <id>
  python -m scripts.meeting search --FIELD VALUE
  python -m scripts.meeting create --name "Intro call" --dateStart "2025-12-01 10:00:00" [--FIELD VALUE]...
"""
...
ENTITY = "Meeting"
SUMMARY_FIELDS = ["id", "name", "status", "dateStart", "dateEnd", "duration", "assignedUserName", "parentType", "parentName"]
...
def create_record(args):
    ...
    while i < len(args):
        if args[i].startswith("--") and i + 1 < len(args):
            ...
            i += 1
    if not payload:
        error("No fields provided. Use --name 'Meeting name' --dateStart '2025-12-01 10:00:00'")
    api_url, api_key, cf_id, cf_secret = get_config()
...
def update_record(args):
    if not args:
        error("Missing meeting ID. Usage: update <id> --field value")
    record_id = args[0]
    payload = {}
    i = 1
    while i < len(args):
...
def delete_record(args):
    if not args:
        error("Missing meeting ID. Usage: delete <id>")
    record_id = args[0]
    api_url, api_key, cf_id, cf_secret = get_config()
