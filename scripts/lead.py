#!/usr/bin/env python3
"""
EspoCRM Lead operations.

Usage:
  python -m scripts.lead list [--limit 20] [--offset 0] [--where FIELD VALUE]...
  python -m scripts.lead get <id>
  python -m scripts.lead search --FIELD VALUE
  python -m scripts.lead create --firstName John --lastName Doe --emailAddress john@example.com [--FIELD VALUE]...
"""
...
ENTITY = "Lead"
SUMMARY_FIELDS = ["id", "firstName", "lastName", "emailAddress", "phoneNumber", "accountName", "status", "source", "city", "country"]
...
def update_record(args):
    if not args:
        error("Missing lead ID. Usage: update <id> --field value")
    record_id = args[0]
    payload = {}
    i = 1
    while i < len(args):
...
def delete_record(args):
    if not args:
        error("Missing lead ID. Usage: delete <id>")
    record_id = args[0]
    api_url, api_key, cf_id, cf_secret = get_config()
